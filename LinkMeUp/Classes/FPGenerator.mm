...
+ (NSString*) generateFingerprintForFile:(NSString*)fullFileName
{
	...
	if (err) {
        NSLog(@"FPGenerator.mm: open failed");
	}
	...
	if (err)
        NSLog(@"FPGenerator.mm:  err on set format %d", err);
	...
	if(bigBuf == NULL) {
        NSLog(@"FPGenerator.mm:  Error mallocing bigbuf");
	}
	...
	while (1) {
		...
		if (err) {
            NSLog(@"FPGenerator.mm  err on read %d", err);
			totalFrames = 0;
			break;
		}
		if (!numFrames)
    ...
    __block NSString *fingerprint;
	if(totalFrames > 11025)
    {
        //NSLog(@"FPGenerator.mm  Starting codegen on %d samples...", totalFrames);
        ...
        //NSLog(@"FPGenerator.mm  Done with codegen");
	}
	free(bigBuf);
